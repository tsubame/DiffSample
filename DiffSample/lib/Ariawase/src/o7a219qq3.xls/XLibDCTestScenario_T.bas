Attribute VB_Name = "XLibDCTestScenario_T"
'概要:
'   DC Test Scenario用カスタムテンプレート
'
'目的:
'   Eee-JOB DCTestScenarioテストインスタンスフレームワーク用に専用テンプレートを作成
'   IG-XL Ver.3.40.17のEmpty_Tをベースにパラメータを追加削除しカスタマイズを行った
'   インスタンスエディタは専用のDCTestScenario_IEを使用する
'
'   現段階ではテンプレートの処理内容は以下の通り
'
'   ①PreBody： TheDCTestオブジェクトの.SetScenarioメソッド呼び出し
'   ②Body：    StartOfBody（引数可）の実行
'               TheDCTestオブジェクトの.Executeメソッド呼び出し
'               EndOfBody（引数可）の実行
'   ③PostBody: 処理なし
'
'   Revision History:
'   Data        Description
'   2008/09/25  技術検討結果としてのカスタムテンプレートプロトタイプ
'   2009/04/07　DCTestScenario_T Ver1.0リリース
'               ■仕様変更
'               ①Eee-JOBライブラリセットのプロパティ･メソッド名称ガイドライン施行に伴い
'               　DCテストシナリオオブジェクト用ラッパーモジュールの関数名が変更されたことによる
'               　テンプレート側呼び出し関数名も変更
'               ②IG-XLライブラリ参照設定の削除
'   2009/04/24　■修正
'               インスタンスエディタのキャプションスペルミスを修正
'   2009/04/27　■修正
'               インスタンスエディタのキャプションスペルミスを修正
'
'作成者:
...
'------------------------------------------------------
'Eee-JOB DCTestScenarioテンプレート用変数追加
...
Private Const ARGNUM_STARTOFBODYF = 0
Private Const ARGNUM_ENDOFBODYF = 1
Private Const ARGNUM_STARTOFBODYFINPUT = 3
Private Const ARGNUM_ENDOFBODYFINPUT = 4
Private Const COMMENTCOLUMN_FOR_EEEJOB = 80
Private Const ARGNUM_MAXARG = ARGNUM_ENDOFBODYFINPUT

Private Const EEEJOB_FORM_CAPTION = "DC Test Scenario Framework -- Instance Editor"
Private Const EEEJOB_TEMPLATE_NAME = "XLibDCTestScenario_T"
Private Const EEEJOB_DEFAULT_COMMENT = "DC Test Scenario Framework For Eee-JOB"
Private Const EEEJOB_INIT_SCENARIO = "XLibDcTestTemplate.SetScenario"
Private Const EEEJOB_EXEC_SCENARIO = "XLibDcTestTemplate.Execute"
'------------------------------------------------------

Function TestTemplate() As Integer
'内容:
'   Eee-JOB DCTestScenarioテンプレート用メイン関数
'
'戻り値：
'   テンプレート実行結果
'   成功：0
...
Function PreBody() As Integer
    ...
    '------------------------------------------------------
    'Eee-JOB DCTestScenarioテンプレート用処理変更
    'JOB側DCテストシナリオエンジン初期化
    'しっくり来ないが現段階ではテラダインAPIでエンジン初期化のラッパー関数を呼び出し
    '初期化で失敗した場合エラーを返す
    PreBody = TheExec.Flow.CallFuncWithArgs(EEEJOB_INIT_SCENARIO, TheExec.DataManager.InstanceName)
    '------------------------------------------------------
    Exit Function
ErrHandler:
    On Error GoTo 0
...
Public Function RunIE(Optional FocusArg As Integer) As Boolean
    ...
    '------------------------------------------------------
    'Eee-JOB DCTestScenarioインスタンスエディタ用処理変更
...
Sub AssignTemplateValues()
    ...
    'Eee-JOB DCTestScenarioテンプレート専用関数に呼び出しを変更
...
'   Eee-JOB DCTestScenarioテンプレート専用関数とした
'
